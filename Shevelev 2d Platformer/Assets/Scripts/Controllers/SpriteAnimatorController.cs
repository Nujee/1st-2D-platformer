...
namespace PlatformerMVC
{
    ...
    public class SpriteAnimatorController : IDisposable
    {
        ...
        private Dictionary<SpriteRenderer, Animation> _aciveAnimations = new Dictionary<SpriteRenderer, Animation>();
        ...
        {
            if(_aciveAnimations.TryGetValue(spriteRenderer, out var animation))
            {
            ...
            {
                _aciveAnimations.Add(spriteRenderer, new Animation()
                {
                    Loop = loop,
                    Speed = speed,
                    Track = track,
        ...
        {
            if (_aciveAnimations.ContainsKey(sprite))
            {
                _aciveAnimations.Remove(sprite);
            }
        }

        public void Update()
        {
            foreach (var animation in _aciveAnimations)
            {
